...
describe('Trade', () => {
  ...
  it('can be constructed with BINANCE as input', () => {
    const trade = new Trade(
      new Route([pair_wbnb_0], BINANCE),
      CurrencyAmount.binance(JSBI.BigInt(100)),
      TradeType.EXACT_INPUT
    )
    expect(trade.inputAmount.currency).toEqual(BINANCE)
    expect(trade.outputAmount.currency).toEqual(token0)
  })
  it('can be constructed with BINANCE as input for exact output', () => {
    const trade = new Trade(
      new Route([pair_wbnb_0], BINANCE, token0),
  ...
  it('can be constructed with BINANCE as output', () => {
    const trade = new Trade(
      new Route([pair_wbnb_0], token0, BINANCE),
      CurrencyAmount.binance(JSBI.BigInt(100)),
      TradeType.EXACT_OUTPUT
    )
    expect(trade.inputAmount.currency).toEqual(token0)
    expect(trade.outputAmount.currency).toEqual(BINANCE)
  })
  it('can be constructed with BINANCE as output for exact input', () => {
    const trade = new Trade(
      new Route([pair_wbnb_0], token0, BINANCE),
      new TokenAmount(token0, JSBI.BigInt(100)),
      TradeType.EXACT_INPUT
  ...
  describe('#bestTradeExactIn', () => {
    ...
    it('works for BINANCE currency input', () => {
      const result = Trade.bestTradeExactIn(
        [pair_wbnb_0, pair_0_1, pair_0_3, pair_1_3],
        CurrencyAmount.binance(JSBI.BigInt(100)),
        token3
      )
      expect(result).toHaveLength(2)
      expect(result[0].inputAmount.currency).toEqual(BINANCE)
      expect(result[0].route.path).toEqual([WBNB[ChainId.MAINNET], token0, token1, token3])
      expect(result[0].outputAmount.currency).toEqual(token3)
      expect(result[1].inputAmount.currency).toEqual(BINANCE)
      expect(result[1].route.path).toEqual([WBNB[ChainId.MAINNET], token0, token3])
      expect(result[1].outputAmount.currency).toEqual(token3)
    })
    it('works for BINANCE currency output', () => {
      const result = Trade.bestTradeExactIn(
        [pair_wbnb_0, pair_0_1, pair_0_3, pair_1_3],
      ...
      expect(result[0].outputAmount.currency).toEqual(BINANCE)
      expect(result[1].inputAmount.currency).toEqual(token3)
      expect(result[1].route.path).toEqual([token3, token1, token0, WBNB[ChainId.MAINNET]])
      expect(result[1].outputAmount.currency).toEqual(BINANCE)
    })
  })
